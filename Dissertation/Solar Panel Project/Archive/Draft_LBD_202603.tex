...
\begin{figure}[tbh]
\centering
\caption{\label{fig:data_sum_two_tech_price_output} Solar Panel Price and Installed Capacity}
\subfloat[Panel Price]
{\centering
...
\end{figure}
Noticeably, the price of c-Si solar panels was notably high and even rising before 2008, but it began to drop rapidly after 2008 and has remained low since. A major factor in this trend is the price of polysilicon, the main component of c-Si solar panels, which experienced a sharp increase in late 2007. This surge was mainly due to the growing demand from the semiconductor sector. 
This situation led to increased investment, which do not depend on polysilicon, and the prices of the primary inputs for this type of solar panels remained quite stable.
This situation led to increased investment, which do not depend on polysilicon, and the prices of the primary inputs for this type of solar panels remained quite stable.
...
\begin{eqnarray}
   TC_{n}(\mathbf{q}_{jnt},s_{jnt},\mathbf{\omega}_{jnt})&=& C_0\!\left(\sum_{m=1}^M q_{jnmt},\; s_{jnt}\right) + \sum_{m=1}^M C_m\!\left(q_{jnmt},\; \omega_{jnmt}\right) \label{eq:model_c_q},
\end{eqnarray}
where the first component, $C_0\!\left(\sum_{m=1}^M q_{jnmt},\; s_{jnt}\right)$, represents the total production cost of $\sum_{m=1}^M q_{jnmt}$, which is a function of the current output and a vector of state variables $s_{jnt}$. These state variables include the firm's accumulated experience $e_{jnt}$, aggregate cost shifters common to all firms (e.g., input price), firm characteristics, and policy indicators that capture the effects of industrial policies on production costs. The second component, $C_m\!\left(q_{jnmt},\; \omega_{jnmt}\right)$, accounts for the additional cost of delivering $q_{jnmt}$ to destination $m$ in year $t$.
...
\end{eqnarray*}
Here, $\gamma_{n}^0$ are technology-specific constants, while the parameters $\gamma_n^s$ capture how state variables affect the unit cost for a specific technology. The parameter $\gamma^{scale}$ captures the curvature of the production function. The parameters $\gamma_{m}^{ex}$ are region-specific and represent the additional cost associated with shipping or exporting to that region. Finally $\omega_{jnmt}$ are cost shocks, which are assumed to follow an independent and identically distributed normal distribution with a mean of zero and a variance of $\sigma_\omega^2$.

As a result, the marginal cost that firm $j$ ships $q_{jnmt}$ to region $m$ is:
\begin{eqnarray}
  \frac{\partial TC_{jnt}}{\partial q_{jnmt}} &=&  \gamma_{n}^0 + s_{jnt} \gamma_n^s + \gamma^{scale} q_{jnmt}  +\gamma_{m}^{ex}  +\omega_{jnmt}. \label{eq:est_mc_q}
\end{eqnarray}
Specially, the vector of state variables $s_{jnt}$ includes the logarithm of one plus the firm's cumulative experience, $ln(e_{jnt}+1)$, the price of polysilicon (the primary input) relevant for c-Si technology, separate time trends for the pre-2010  and post-2010 periods, and the firm's ownership status.

%Here, the vector of state variables $s_{jnt}$ includes the firm's cumulative experience $e_{jnt}$, along with other firm characteristics such as ownership status. In addition, it includes technology-specific cost shifters that impact all firms, such as prices of major inputs, and policy variables that reflect the impact of industrial policies on production costs. As mentioned in Section \ref{subsec:inst_china_PV}, the central government has been directly supporting this industry \textcolor{blue}{since 2009}, and several provincial governments also provide generous support. To capture these policy impacts, we incorporate indicators for the year 2009 onward and for the Zhejiang province. 
...
%where $\gamma_{n}^0$ are technology-specific intercepts; $\gamma_n^{\text{exp}}$ measure the impact of learning-by-doing on marginal cost for each technology, with $e_{jt}^{GW} = \sum_{t' < t} q_{jt'} / 10^9$ denoting the firm's cumulative production experience in gigawatts; $\gamma^{\text{poly}}$ captures the elasticity of marginal cost with respect to the polysilicon spot price $P_t^{\text{poly}}$, which applies only to crystalline silicon firms; $\gamma^{\text{SOE}}$ captures the cost advantage of state-owned enterprises; $\gamma^{\text{time}}$ is a period adjustment for the years 2006--2008; $\gamma^{\text{trend}}$ captures secular annual cost decline beyond firm-specific learning (e.g., industry-wide technological progress and supply chain maturation); the Zhejiang province indicator reflects the cost advantage associated with the industry cluster in that province; $\gamma_{m}^{\text{ex}}$ are parameters capturing the cost differential associated with exporting to market $m$, weighted by the firm's export share $s_{jt}^m$ to that market; $\gamma^{\text{scale}}$ introduces a convex cost component reflecting firm-level capacity constraints, so that marginal cost is increasing in own output; and $\omega_{jt}$ are firm-level cost shocks assumed to follow an i.i.d.\ normal distribution with mean zero and variance $\sigma_\omega^2$.

%The log specification for experience, $\ln(e_{jt}^{GW}+1)$, captures diminishing returns to learning-by-doing: early production experience yields larger cost reductions than later experience. An alternative linear specification is also tested, with the preferred specification selected via calibration.
...
\quad Scale ($\gamma^{scale}$) & 5.007 & (0.010) \\
\quad SD of shocks ($\sigma^{\omega}$) & 4.834 & (0.119) \\
\bottomrule
\end{tabularx}
\begin{tablenotes}
...
Table \ref{tab:est_dyn_para_2} presents the estimated standard deviation of scrap value and the mean entry costs for the two technologies. As discussed in Section \ref{sec:institution_data}, the number of new entrants increases sharply after 2010, prompting us to differentiate c-Si firms' entry costs before and after 2010. Our estimates indicate that the entry cost for c-Si firms was roughly 1.75 \textcolor{blue}{million RMB} lower prior to 2010, representing an estimate of the overall entry subsidy before that year.
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{Figures/industry_fit_dynamic_foc.png}
    \caption{Overall model fit.}
    \label{fig:placeholder}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{Figures/marginal_cost_over_time.png}
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{Figures/production_fit_dynamic_foc.png}
    \caption{Production Model Fit.}
    \label{fig:placeholder}
\end{figure}
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{Figures/baseline_fit.png}
    \caption{Aggregate Production Model Fit.}
    \label{fig:placeholder}
\end{figure}
...
\begin{itemize}
    \item CF1-0: feed-in tariff and tax credits + no LBD
    \item CF1-1: feed-in tariff and tax credits + LBD
    \item CF2-0: import tariffs + no LBD
    \item CF2-1: import tariffs + LBD
\end{itemize} 
...
We examine the effect of postponing the demand policies until 2010.

We examine the effect of implementing the production policies earlier by five years (shifting it from 2010 on ward to 2005.

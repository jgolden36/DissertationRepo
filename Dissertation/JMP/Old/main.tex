...
%I find that a 3000 megawatt HVDC line between Texas and New York would lead to a 15 percent decrease in electricity prices in New York and a 10 percent increase in prices in Texas leading to a 20 percent increase in renewable energy production in Texas and a 40 percent decrease in fossil fuel electricity production in New York. Based on this outcome, building the line would create a decline in total emissions of 10 megatons of carbon dioxide over ten years. The model also finds a 2 percent increase in blackouts in New York over the period due to the loss of fossil fuel generators. Including producer profits and increasing consumer welfare, the net present value of the HVDC line is 250 million.
...
Table \ref{ResourcesTable} shows results for the short-run model for export sensitivity and base cost parameters for all resources except solar, hydro, and wind. Table \ref{NaturalGasTable} shows the coefficients for important natural gas characteristic coefficients while Table \ref{CoalTable} shows the same for coal resources. There is a positive impact of  

Figure \ref{fig:ModelFit} shows model fit for the short-run model. The model relatively consistently underestimates the price average by start zone code but correctly matches dispersion for the market. The scale of 
...
Table \ref{longRunResults} provides results for the long-run model with each of the 32 parameters estimated alongside significance and standard errors.

Figure \ref{fig:longrunModelFit} showcases model fit for the long-run model in terms of total grid capacity during the period for which we have data. The total stock of generation capacity matches reasonably well, but the investment patterns differ significantly with the model tending to overestimate the investments in solar and wind and estimate nearly flat coal and natural gas capacities. 
...
Counterfactuals are run for the short-run and long-run model. For the short run model, the model is simulated with replacement of all lines with HVDC lines as well as partial replacement of a percentage of randomly chosen transmission lines. We also add additional HVDC lines between specific zones to simulate specific improvements in the grid. Table \ref{} provides the reduction in average prices related to different percentages of upgrades to lines in the grid. Figure \ref{priceDecrease} shows the impacts of upgrades on average price for select percentage upgrades of grid infrastructure. The highest LMP decline is seen with full replacement of all lines, but almost the entire reduction in LMP can be found from a 
...
I also propagate the counterfactuals from the short-run to the long-run to provide evidence on the alteration of the path of capacity investments with changes in the grid. This allows for an understanding of the dynamic effects of structural changes in the grid. I find that with improvements of lines to HVDC, investment in renewables increases, and retirements of fossil fuel generators also increases. The same happens when lines are added between new regions. Ultimately, expansion of transmission leads to a shift in generation capacity investments towards renewables-rich regions in the Midwest. This is exactly what the model predicts.

\section{Conclusion} \label{sec:conclusion}

I detail and estimate model of generator decision-making in the electricity market in the presence of transmission constraints. I estimate the impacts of increasing transmission constraints on prices and on investments in generation. I find that increasing transmission capacity leads to a decrease in prices overall and leads to an increase in investments in renewable energy, which increases the speed of the transition from fossil fuel to renewable energy.

The current work has numerous limitations. Most importantly, the model is only at the zonal level and not at the nodal level. This prevents the analysis from truly understanding the impacts of adding transmission as individual lines would connect individual substations and not regions. The model is thus more useful as an estimate of the dynamic effects of large-scale interregional transmission, and the assumptions made in application to a physical system should be clear when using the model for analysis. Further, the model does not take into account the shifts in demand related to changes in the cost of power or the effects of 

Future work should expand on the current work by integrating nodal-level data to analyze the impact of additions of individual lines. Similarly, future work should attempt to endogenize demand as long-run shifts in the costs of power will certainly shift the geographic composition of demand. 

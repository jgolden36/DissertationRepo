...
Figure \ref{fig:NetworkTopology} illustrates the mechanics of congestion pricing in a simplified three-zone network. When transmission constraints bind, cheap power from low-cost zones cannot fully reach high-demand areas, causing locational marginal prices to diverge. Each zone's LMP equals the local marginal cost of generation when export constraints are binding.
...
% Arrows between dispatch
\draw[arrow] (st1) -- (st2);
\draw[arrow] (st2) -- (st3);
\draw[arrow] (st3) -- (dots1);

% Transmission constraints feed into dispatch
\draw[dasharrow, color=blue!60] (transmission) -- (st1);
\draw[dasharrow, color=blue!60] (transmission) to[bend left=15] (st2);
\draw[dasharrow, color=blue!60] (transmission) to[bend left=25] (st3);

% Signal node
\node (signal) [signal, below=1.2cm of st2] {Prices, Costs\\$\rightarrow$ Expected $\pi$};

% Arrows from dispatch to signal
\draw[arrow, color=blue!60] (st1.south) to[out=-90, in=150] (signal.north west);
\draw[arrow, color=blue!60] (st2.south) -- (signal.north);
\draw[arrow, color=blue!60] (st3.south) to[out=-90, in=30] (signal.north east);

% Long-run row
\node (lt1) [invest, below=1.2cm of signal, xshift=-2.5cm] {Capacity\\Investment};
\node (lt2) [invest, right=0.8cm of lt1] {Capacity\\Investment};
\node (lt3) [invest, right=0.8cm of lt2] {Capacity\\Investment};
...
% Arrows between investment
\draw[arrow] (lt1) -- (lt2);
\draw[arrow] (lt2) -- (lt3);
\draw[arrow] (lt3) -- (dots2);

% Signal feeds investment
...
\end{tikzpicture}
\caption{Timeline of Short-run and Long-run Model Interaction. The ISO solves optimal dispatch hourly subject to transmission constraints. Short-run prices and costs determine expected profits, which drive long-run capacity investment decisions. Updated capacity then affects future dispatch outcomes.}
